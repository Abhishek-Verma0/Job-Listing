...
const Input: React.FC<InputProps> = ({label, id,type,register,errors,disabled,noLabel,isTextArea,placeHolder,noBorder,onChange,value}) => {

    const inputElement = isTextArea ? (
        <textarea id={id} autoComplete={id} rows={6} disabled={disabled} placeholder={placeHolder} {...register} className={clsx(
        ...
        <input 
         id={id} type={type} autoComplete={id} name={id} value={value} disabled={disabled} placeholder={placeHolder} {...register} onChange={onChange} className={clsx(
            `block w-full rounded-md border-0 py-1.5 px-1 text-gray-900 shadow-md ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:outline-purple-600 sm:text-sm sm:leading-6 
            `, 
            errors && errors[id] && "ring-rose-700" , 
            disabled && "opacity-50 cursor-default",
